...
common_text = "\n\n<u>If you facing any problem, so report at @AJBotVerseGroup</u>"
start1 = "<i>To login your account send login detail in this format email,password.</i>\n"
start_msg = f"<b>I am a Bot.</b>\n\n{start1}\nWe will store your login detail for 24 hour only, after that to you need to login again to use it.{common_text}"
...
async def search_user_in_channel(userid):
    ...
    users1 = await client.get_participants('AJBotVerse')
    users2 = await client.get_participants('AJBotVerseGroup')
    for user in users1:
        if str(user.id) == str(userid):
            for userg in users2:
                if str(userg.id) == str(userid):
                    return True
    await bot.send_message(int(userid), not_joined_channel, buttons = [Button.url('Join our Channel.','https://t.me/AJBotVerse'), Button.url('Join our Group.','https://t.me/AJBotVerseGroup')])
    return False
